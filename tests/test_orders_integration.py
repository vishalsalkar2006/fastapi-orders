...
def test_update_order_not_found():
    updated_data = {
        ...
        "product_id": 2,
        "quantity": 1,
        "total_price": 120.0,  # ✅ corrected field name
        "status":"Pending"
    }
...
def test_partial_update_order(db):
    ...
    response = client.patch(f"/orders/{order.id}", json=partial_data)  # ✅ PATCH for partial update
    assert response.status_code == 200
    data = response.json()

    assert data["quantity"] == 5
    assert data["customer_name"] == "Partial Update"  # unchanged
    assert data["total_price"] == 200  # unchanged
